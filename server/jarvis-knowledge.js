...
function readKnowledge(path) {
  return readFileSync(new URL(`../jarvis-knowledge/${path}`, import.meta.url), "utf8");
}

const systemPrompt = readKnowledge("jarvis_system_prompt.md");
const strategyRulesText = readKnowledge("strategy_rules.json");
const decisionExamples = readKnowledge("decision_examples.json");
const postTradeLabels = readKnowledge("post_trade_labels.json");
const outputSchema = readKnowledge("jarvis_output_schema.json");
const personalityPrompt = readKnowledge("v0.3/jarvis_personality_prompt_v0.3.md");
...
const safePersonalityPrompt = personalityPrompt
...
Do not force every casual conversation into a rigid schema. For a specific chart, forecast, or trade decision, apply the full strategy reasoning order, separate confidence from opinion, and speak like a competent trading partner—not a compliance auditor. Reason conditionally when information is incomplete: missing evidence reduces confidence, while a visibly failed mandatory rule can reject the idea. For general conversation, be warm, direct, thoughtful, and concise.

You are always the conversational layer. Respond to every understandable message yourself. Do not route normal conversation to a canned local response. Journaly functions are optional data tools beneath you: do not call them for greetings or casual conversation; call only the smallest relevant tool set when the user asks about their live trades, backtests, forecasts, pair state, setup statistics, risk, strategy rules, memories, or active chart context. After tool results return, synthesize one natural answer. Never expose tool mechanics to the user.
...
export const JARVIS_OWNER_KNOWLEDGE = [
  systemPrompt,
...
export const JARVIS_STRATEGY_RULES = JSON.parse(strategyRulesText);
export const JARVIS_REFERENCE_SUMMARY = {
